...
app
	...
	.then(() => {
		...
		server.use(
			express.static(__dirname + "/../nokia_bell_labs_deploy/shared/assets")
		);
		...
		server.post("/transfer", (req, res) => {
			...
			if (req.body.filename.split(".").pop() != "mp4") {
				fs.rename(
					__dirname + "/static/tmp/" + req.body.filename,
					__dirname +
						"/../nokia_bell_labs_deploy/shared/assets/img/" +
						req.body.filename,
					(err) => {
						if (!err) {
				...
				fs.rename(
					__dirname + "/static/tmp/" + req.body.filename,
					__dirname +
						"/../nokia_bell_labs_deploy/shared/assets/video/" +
						req.body.filename,
					(err) => {
						if (!err) {
							res.json({ success: true, request: req.body });
		...
		server.post("/temp-upload", tempUpload.single("media"), (req, res) => {
			//console.log('/upload', new Date());
			//console.log(req.file);
			if (req.file) {
				//console.log('success!');
		...
		server.post("/save/:type", (req, res) => {
			...
			if (req.params.type == "live") {
				...
				if (req.params.type == "temp-saved")
					filepath = dir + date + time + " (Saved).json";
				fs.writeFile(filepath, data, (err) => {
				...
				if (req.params.type == "temp-saved") {
					...
					request.post(
						IP_CONFIG["overhang"] + "/preview-content",
						{
							form: {
								content: JSON.stringify(JSON.parse(req.body.data), null, 2),
							},
						},
						(err, res, body) => {
							//console.log(body);
					...
					for (var i = 1, len = 3 + 1; i < len; i++) {
						...
						if (i === 3) {
							request.post(
								IP_CONFIG[touchtable_id] + "/save-content",
								{
									form: {
										content: JSON.stringify(JSON.parse(req.body.data), null, 2),
									},
							...
							setTimeout(() => {
								...
								exec(
									'git add -A && git commit -m "client SAVED content.json" && git push',
									{ cwd: __dirname },
									function (err, stdout, stderr) {
										console.log("done");
									}
							...
							request.post(
								IP_CONFIG[touchtable_id] + "/preview-content",
								{
									form: {
										content: JSON.stringify(JSON.parse(req.body.data), null, 2),
									},
			...
			res.json({
				received: JSON.parse(req.body.data) || "blank",
		...
		IO_CLIENT.overhang = socketIO_client.connect(
			IP_CONFIG.overhang + "/overhang"
		);
		...
		for (var i = 1, len = 3 + 1; i < len; i++) {
			...
			IO_CLIENT[touchtable_id] = socketIO_client.connect(
				IP_CONFIG[touchtable_id] + "/touch"
			);

			IO_CLIENT[touchtable_id].on(
				"connect",
				(function (index) {
					return () => {
