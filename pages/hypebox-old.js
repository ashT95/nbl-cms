...
class Hypebox extends React.Component {
	...
	getCurrContentItem(){
		return this.getContentItems()[this.state.contentItem_index] || {text:'', figure:''};
	}
									...
									<ul>
										{ 
											this.getContentItems().map((item,index)=>{
												let type = item.figure != undefined ? 'image' : 'text';
						...
						<div className='col right'>
							...
							<div className='row btm' id='ContentItemData'>
								<section>
									...
									<h3>{this.getContentItemType(this.state.contentItem_index) == 'text' ? 'text' : 'image caption'}</h3>
